...
class VCardsTableSeeder extends Seeder
{
    ...
    private function getRandomPayment($faker, $originalVCardPhoneNumber, $type, &$paymentType, &$paymentRef)
    {
        ...
        $paymentType = $type == 'C' ? PaymentTypesTableSeeder::getRandomCredit() : PaymentTypesTableSeeder::getRandomDebit();
        $paymentRef =  PaymentTypesTableSeeder::getRandomRef($faker, $originalVCardPhoneNumber, $paymentType);
    }
    ...
    private function updateSoftDeletesTransactions($faker)
    {
        ...
        foreach ($vcardsToClose as $vcard) {
            ...
            DB::update(
                'update categories set deleted_at = ?, updated_at=? where vcard=?',
                [
                    $vcard['deleted_at'],
                    $vcard['deleted_at'],
                    $vcard['phone_number']
